...
from hfm.utils.verifiers import check_zero  # DTY_PLT
...
# |             |  correct | incorrect |
# |    correct  |   e_{00} |  e{01}    |
# |  incorrect  |   e_{10} |  e{11}    |
#
def contingency_tab_multiclass(ha, hb, y):
    # Do NOT use this function to calcuate!
    a = np.sum(np.logical_and(
        np.equal(ha, y), np.equal(hb, y)))          # e_{00}
    c = np.sum(np.logical_and(
        np.not_equal(ha, y), np.equal(hb, y)))      # e_{01}
    b = np.sum(np.logical_and(
        np.equal(ha, y), np.not_equal(hb, y)))      # e_{10}
    d = np.sum(np.logical_and(
        np.not_equal(ha, y), np.not_equal(hb, y)))  # e_{11}
