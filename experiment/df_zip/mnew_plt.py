...
class GraphSetup(GraphSetupVer2):
    ...
    def sub_dat_4tentative(self, dframe, id_set, pos, tag_sa1, tag_sa2,
                           binary=False):  # pos is `specific distance`
        columns = {t2: t1 for t1, t2 in zip(tag_sa1, tag_sa2)}

        if binary:
            df_raw = dframe.iloc[id_set[1] + 1:id_set[2]]
            df_raw = df_raw[tag_sa2].rename(columns=columns)
            start = id_set[1] + 1 + pos * self._nb_iter
            df_raw = df_raw.loc[start: start + self._nb_iter - 1]
            for k in [1, 2, 3]:
                df_tmp = dframe.iloc[id_set[k] + 1: id_set[k + 1]][tag_sa1]
...
class DistPerf_draw(GraphSetup):
    ...
    def prepare_graph(self, n_l):
        n_ell = (1 + n_l) * 2 * 3
        csv_row_1 = unique_column(9 + n_ell * 2)
        # params = csv_row_1[: 9]
        tag_sa1 = csv_row_1[9: 9 + n_ell]
        tag_sa2 = csv_row_1[9 + n_ell:]

        n_ell = (1 + n_l) * 2
        tag_tim = [tag_sa1[:n_ell], tag_sa2[:n_ell]]
        tag_d_max = [tag_sa1[n_ell: -n_ell], tag_sa2[n_ell: -n_ell]]
        tag_d_avg = [tag_sa1[-n_ell:], tag_sa2[-n_ell:]]
        ...
        # pdb.set_trace()
        return tag_d_max, tag_d_avg, tag_tim, picked_m
...
class cvgPlt1C_take(DistPerf_draw):
    def prepare_graph(self):
        csv_row_1 = unique_column(9 + 78)
        # tag_pm = csv_row_1[:9]
        tag_sa1 = csv_row_1[9: 9 + 26]
        tag_sa2 = csv_row_1[9 + 26: 9 + 26 * 2]
        tag_wh = csv_row_1[9 + 26 * 2:]
        # tag_tim = tag_sa1[:10] + tag_sa2[:10]
        # tag_d_max = tag_sa1[10:20] + tag_sa2[10:20]
        # tag_d_avg = tag_sa1[20:] + tag_sa2[20:]
        return tag_wh, tag_sa1, tag_sa2

    def schedule_mspaint(self, raw_dframe, pre='minmax'):  # ,pos=1):
        nb_set, id_set, _, _, _ = self.recap_sub_data(
            raw_dframe, nb_row=4, nc_norm=6, nc_sens=0)
        tag_wh, tag_sa1, tag_sa2 = self.prepare_graph()
        # df_raw = self.obtn_sa_multivar(raw_dframe, id_set, pos, tag_sa1, tag_sa2)
        # # df_tmp = self.obtn_sa_multivar(raw_dframe, id_set, 1, tag_wh, [])
        # # fgn = self._figname + f'_{pre}'
        # # self.sub_plt_tim(df_raw, tag_sa1[:10], fgn + '_sa1', 't_D')
        # df_raw = self.sub_dat_4tentative(
        #     raw_dframe, id_set, pos, tag_sa1, tag_sa2, True)

        fgn = f'{self._figname}_{pre}'  # '_pos'
        for pos in [1, 2, 3, 4, 5]:
            df_raw = self.sub_dat_4tentative(
                raw_dframe, id_set, pos, tag_sa1, tag_sa2)
            self.sub_plt_curr_dist(df_raw, tag_sa1, f'{fgn}_non_pos{pos}')
            # self.sub_plt_curr_dist(df_raw, tag_sa1, fgn+str(pos)+'_non')
            # df_raw = self.sub_dat_4tentative(
            #     raw_dframe, id_set, pos, tag_sa1, tag_sa2, True)
            # self.sub_plt_curr_dist(df_raw, tag_sa1, f'{fgn}{pos}_bin')
        df_raw = self.sub_dat_4tentative(
            raw_dframe, id_set, 1, tag_sa1, tag_sa2, True)
        self.sub_plt_curr_dist(df_raw, tag_sa1, f'{fgn}_bin_pos1')

    def sub_plt_curr_dist(self, df_raw, tag_sa1, fgn):
        tag_tim, tag_d_max, tag_d_avg = tag_sa1[:10], tag_sa1[10:20], tag_sa1[20:]
        ...
        tag_compare = tag_d_max[5:6] + tag_d_max[7:]  # EffHD_nonbin
        tag_compare = tag_d_max[1:2] + tag_d_max[7:]  # EffHD_bin
        tag_earlybreak = tag_tim[1:2] + tag_tim[7:8]  # +tag_tim[3:4]
        # pdb.set_trace()

        '''
        multi_lin_reg_wo_distr_gather(
            [df_raw[tag_d_max[6]].values.astype(DTY_FLT),
             df_raw[tag_d_avg[2]].values.astype(DTY_FLT),
             df_raw[tag_tim[6]].values.astype(DTY_FLT),
             df_raw[tag_tim[6]].values.astype(DTY_FLT), ],
            [df_raw[tag_compare].values.astype(DTY_FLT).T,
             df_raw[tag_d_avg[3:]].values.astype(DTY_FLT).T,
             df_raw[tag_earlybreak].values.astype(DTY_FLT).T,
             df_raw[tag_tim[7:]].values.astype(DTY_FLT).T, ],
            ...
            [annots_sub1, annots_sub2, annots_sub3, annots_sub3],
            fgn.replace('_strat', '_strat_org'),  # fgn,
            snspec=['sty3b', 'sty3c', 'sty4', 'sty4c'], strt=0)  # 'sty4e'
        '''

        tim_part_x = df_raw[tag_tim[6]].values.astype(DTY_FLT)
        tim_part_ya = df_raw[tag_earlybreak].values.astype(DTY_FLT).T
        tim_part_yb = df_raw[tag_tim[7:]].values.astype(DTY_FLT).T
        tim_part_ya = np.log10(tim_part_ya / tim_part_x)
        tim_part_yb = np.log10(tim_part_yb / tim_part_x)
        annots_sub3[1] = r'$\lg( \frac{ T_{\hat{\mathbf{D}}_{\cdot,\mathbf{a}}(S,a_i)} }{ T_{\mathbf{D}_{\cdot,\mathbf{a}}(S,a_i)} } )$'
        # pdb.set_trace()
        multi_lin_reg_wo_distr_gather(
            [df_raw[tag_d_max[6]].values.astype(DTY_FLT),
             df_raw[tag_d_avg[2]].values.astype(DTY_FLT),
             tim_part_x, tim_part_x, ],
            [df_raw[tag_compare].values.astype(DTY_FLT).T,
             df_raw[tag_d_avg[3:]].values.astype(DTY_FLT).T,
             tim_part_ya, tim_part_yb, ],
            [['EarlyBreak', 'ApproxDist', 'StratES', 'StratRA'],
             ['ApproxDist', 'StratES', 'StratRA'],
             [r'$T_{EarlyBreak}$', r'$T_{ApproxDist}$'],
             [r'$T_{ApproxDist}$', r'$T_{StratES}$', r'$T_{StartRA}$'], ],
            [annots_sub1, annots_sub2, annots_sub3, annots_sub3],
            fgn + '_strat',  # fgn.replace('_strat', '_strat_alt'),
            snspec=['sty3b', 'sty3c', 'sty6', 'sty6c'], strt=0)
        return

    # def sub_plt_tim(self, df_tmp, tag, fgn, sgn='t_D.'):  # ,eb=True):
    #     # pdb.set_trace()
    #     scat_X = df_tmp[tag[4 + 2]].values.astype(DTY_FLT)
    #     scat_Y = [df_tmp[tag[5]].values.astype(DTY_FLT),
    #               df_tmp[tag[7]].values.astype(DTY_FLT),
    #               df_tmp[tag[8]].values.astype(DTY_FLT),
    #               df_tmp[tag[9]].values.astype(DTY_FLT)]
