...
def situation_split2(pr_trn, nb_iter, y_trn):
    ...
    dY = len(vY)
    iY = [np.where(np.array(y_trn) == j)[0] for j in vY]
    lY = [len(j) for j in iY]  # length
    sY = [int(
        np.max([np.round(j * pr_trn), 1])) for j in lY]
...
def situation_cross_validation(nb_iter, y,
    ...
    # tY = [np.arange(j) for j in lY]  # temp_index
    tY = [np.copy(j) for j in iY]
    for j in tY:
        np.random.shuffle(j)
    sY = [int(np.floor(j / nb_iter)) for j in lY]
    if nb_iter == 2:
    ...
    for k in range(1, nb_iter + 1):
        ...
        if split_type.endswith("v2"):
            ...
            # temp_ = (i_trn.copy(), i_val.copy(), i_tst.copy())
...
def according_index_split_train_valid_test(X, y, split_idx_item):
    ...
    return X_trn.tolist(), X_val.tolist(), X_tst.tolist(), \
        y_trn.tolist(), y_val.tolist(), y_tst.tolist()  # .copy()
...
def cross_validate_training_test_set(y, X=None, cv=5):
    if X is None:
        ...
        kf.get_n_splits(y)
        split_idx = [(i.tolist(),
                      j.tolist()) for i, j in kf.split(y)]
    else:
        kf = model_selection.StratifiedKFold(n_splits=cv)
        kf.get_n_splits(X)
        split_idx = [(i.tolist(),
                      j.tolist()) for i, j in kf.split(X, y)]
    return split_idx
